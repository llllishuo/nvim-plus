...
local function set_keymap()
	...
	map("n", keys.write_quit_all, ":wqa!<CR>", option)
	map("n", keys.quit, ":q!<CR>", option)
	map("n", keys.write_quit, ":wq!<CR>", option)
	map("n", keys.save, ":wa!<CR>", option)

	-- 上下移动四行
	map("n", keys.move_4j, "4j", option)
	map("n", keys.move_4k, "4k", option)

	-- 左右移动标签栏页
	map("n", keys.move_tab_right, ":BufferLineCycleNext<CR>", option)
	map("n", keys.move_tab_left, ":BufferLineCyclePrev<CR>", option)
	-- 关闭当前标签页
	map("n", keys.delete_tab_now, ":Bdelete!<CR>", opt)
	...
	local bottom_terminal_default = require("toggleterm.terminal").Terminal:new({
		...
		on_open = function(term)
			...
			-- set keymapping
			local opts = { buffer = 0 }
			vim.api.nvim_buf_set_keymap(term.bufnr, "t", "", [[<C-\><C-n>]], { noremap = true, silent = true })
